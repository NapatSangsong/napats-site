	...
	return (
		<section>
			...
			<div className="bar-label">
				...
				<span className="mono">ส้ม = ชั่วโมงที่หนักสุด (top 20%) · ชี้เพื่อดูตัวเลข</span>
			</div>
			{dates.map((day) => {
					...
					<div className="hrow" key={day}>
						...
						<div className="hcells">
							{hours.map((h) => {
								...
								if (v === undefined) {
									return <div key={h} className="hcell hempty" title="ไม่มีข้อมูล" />;
								}
								const r = v / mx;
								const color =
									r > 0.8
								...
								return (
									<div
										...
										title={`${dayMonth(day)} ${String(h).padStart(2, "0")}:00 · ${f2(v)} kWh`}
									/>
								);
							})}
						</div>
