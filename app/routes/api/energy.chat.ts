...
function systemPrompt(context: string): string {
	return [
		...
		"ข้อมูลด้านล่างมีตารางเปรียบเทียบทุก scenario (Flat/TOU ±โซลาร์ 2k/4k) + พารามิเตอร์โมเดลครบ",
		"ถ้าถูกถามคอมโบที่ไม่มีในตาราง (เช่น โซลาร์ 6kW, flat+แบต, ย้ายโหลดบางส่วน) ให้ **คำนวณเองจากพารามิเตอร์** แล้วตอบเป็นตัวเลข ฿/เดือน พร้อมแสดงวิธีคิดสั้นๆ 1–2 บรรทัด",
		"วิธีคิด flat = ขั้นบันไดบนหน่วย grid + Ft×หน่วย แล้ว ×VAT 1.07 + ค่าบริการ · TOU = on×5.81 + off×2.99 + ค่าบริการ · โซลาร์ตัด min(ผลิต, โหลดกลางวัน)×จำนวนวัน แล้วบวก subscription",
		"ห้ามแต่งพารามิเตอร์/เรตที่ไม่มีในข้อมูลขึ้นเอง — ถ้าขาดข้อมูลจริงๆ ค่อยบอกว่าไม่มี",
		"",
